...
class UserController {
  ...
  async userJobApply(req, res) {
    try {
      const { description, jobId, phoneno, username, email } = req.body;
      const { userId } = req.user;
      const cv = req.file ? `/uploads/${req.file.filename}` : null;

      // Validation
      if (!jobId || !description || !cv || !username || !email || !phoneno) {
        return res
          .status(400)
      ...
      // Add application
      job.userApply.push({ userId, username, email, cv, description, phoneno });
      await job.save();
